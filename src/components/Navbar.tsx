    ...
    <>
      ...
      >
        <motion.nav
          ...
          animate={{
            backgroundColor: shrunk ? 'rgba(245,241,232,0.82)' : 'rgba(245,241,232,0.4)',
            backdropFilter: shrunk ? 'blur(16px)' : 'blur(8px)',
            boxShadow: shrunk
              ? '0 2px 24px rgba(5,5,5,0.07), inset 0 0 0 1px rgba(5,5,5,0.06)'
              : '0 1px 12px rgba(5,5,5,0.03), inset 0 0 0 1px rgba(5,5,5,0.04)',
          }}
          transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
        >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-x-0 top-16 z-[99] mx-3 rounded-2xl border border-ink/10 bg-bg/95 p-6 shadow-2xl backdrop-blur-xl md:hidden"
          >
            <nav className="flex flex-col gap-4">
              ...
              <a
                href="/#contact"
                onClick={() => setMobileMenuOpen(false)}
                className="mt-2 flex items-center justify-center gap-2 rounded-xl bg-ink py-3 font-display text-xs font-bold uppercase tracking-[0.2em] text-bg"
              >
                Let's Talk ↗
              </a>
            </nav>
